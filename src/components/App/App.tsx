...
const App = () => {
    ...
    const [] = useDebounce(() => {
        getCoords(firstPoint).then((coords) => setFirstCoord([coords?.longitude!, coords?.latitude!]));
    }, 1000, [firstPoint]);

    const [] = useDebounce(() => {
        getCoords(secondPoint).then((coords) => setSecondCoord([coords?.longitude!, coords?.latitude!]));
    }, 1000, [secondPoint]);
    ...
    return (
        <div className={styles.app}>
            ...
            <MapTest
                ...
                routePoints={[firstCoord, secondCoord]}
            />
        </div>
    );
};
